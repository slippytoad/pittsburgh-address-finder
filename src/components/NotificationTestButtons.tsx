...
import { Mail, Send, Play, Trash2, MessageSquare } from 'lucide-react';
...
const NotificationTestButtons: React.FC = () => {
  ...
  const sendTestEmail = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('daily-violation-check', {
        body: { test_run: true, test_type: 'email' }
      });
  ...
  const sendTestSms = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('daily-violation-check', {
        body: { test_run: true, test_type: 'sms' }
      });
      ...
      toast({
        title: "Test SMS Sent",
        description: "Test SMS has been sent successfully. Check your phone!"
      });
    } catch (error) {
      console.error('Error:', error);
      toast({
  ...
  const sendDailyReport = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Daily violation report has been sent successfully. Check your notifications!"
      });
    } catch (error) {
      console.error('Error:', error);
      toast({
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Notification Testing & Data Management
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded-md">
          ...
          <strong>Test SMS:</strong> Sends a simple test message to verify SMS functionality.<br/>
